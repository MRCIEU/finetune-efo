...
ignore <- scan(url("https://raw.githubusercontent.com/MRCIEU/genotype-phenotype-map/refs/heads/main/pipeline_steps/data/ignore_studies_rare.tsv?token=GHSAT0AAAAAAC5KCQIGBVFJEIUFEAJNWPAMZ5ZAVNQ"), what="character")

dat <- dat %>% filter(! study_name %in% ignore)


str(dat)

dat %>% filter(data_type == "phenotype") %>% group_by(data_type, variant_type, source) %>% summarise(n = n())
dat <- subset(dat, data_type == "phenotype" & source != "ebi_catalog" & variant_type == "rare_exome")

sourcestring1 <- "az_exwas"
sourcestring2 <- "backman"

do_matches <- function(dat, sourcestring1, sourcestring2) {
    match_exact <- merge_plus(
        subset(dat, source == sourcestring1) %>% select(study_name1 = study_name, trait, source, sample_size),
        subset(dat, source == sourcestring2) %>% select(study_name2 = study_name, trait, source, sample_size),
        by=c("trait"),
        suffixes=c("_1", "_2"),
        unique_key_1="study_name1",
        unique_key_2="study_name2",
        match_type = "exact"
    )
    dat2 <- subset(dat, ! study_name %in% match_exact$matches$study_name1)

    match_fuzzy <- merge_plus(
        subset(dat2, source == sourcestring1) %>% select(study_name1 = study_name, trait, source, sample_size),
        subset(dat2, source == sourcestring2) %>% select(study_name2 = study_name, trait, source, sample_size),
        by=c("trait"),
        suffixes=c("_1", "_2"),
        unique_key_1="study_name1",
        unique_key_2="study_name2",
    ...
    dat3 <- subset(dat2, ! study_name %in% match_fuzzy$matches$study_name1)
    ...
    match_fuzzy2 <- merge_plus(
        subset(dat3, source == sourcestring1) %>% select(study_name1 = study_name, trait, trait2, source, sample_size),
        subset(dat3, source == sourcestring2) %>% select(study_name2 = study_name, trait, trait2, source, sample_size),
        by=c("trait2"),
        suffixes=c("_1", "_2"),
        unique_key_1="study_name1",
        unique_key_2="study_name2",
    ...
    dat4 <- subset(dat3, ! study_name %in% match_fuzzy2$matches$study_name1)

    match_fuzzy3 <- merge_plus(
        subset(dat4, source == sourcestring1) %>% select(study_name1 = study_name, trait, trait2, source, sample_size),
        subset(dat4, source == sourcestring2) %>% select(study_name2 = study_name, trait, trait2, source, sample_size),
        by=c("trait2"),
        suffixes=c("_1", "_2"),
        unique_key_1="study_name1",
        unique_key_2="study_name2",
    ...
    dat5 <- subset(dat4, ! study_name %in% match_fuzzy3$matches$study_name1)
